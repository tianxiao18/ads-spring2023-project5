...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    ...
    # ------- train_loader -------
    #print(torch.from_numpy(train_x).type(torch.FloatTensor))
...
def compute_val_loss_mstgcn(net, val_loader, criterion,  masked_flag,missing_value,sw, epoch, limit=None):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            labels_shape[-1] = 1
            labels_shape = tuple(labels_shape)
            labels = labels[:,:,-1].reshape(labels_shape)
            if masked_flag:
    ...
    with torch.no_grad():
        ...
        for i in range(prediction_length):
            ...
            if metric_method == 'mask':
                mae = masked_mae_test(data_target_tensor[:, :, -1], prediction[:, :, i],0.0)
                rmse = masked_rmse_test(data_target_tensor[:, :, -1], prediction[:, :, i],0.0)
                mape = masked_mape_np(data_target_tensor[:, :, -1], prediction[:, :, i], 0)
            else :
                mae = mean_absolute_error(data_target_tensor[:, :, -1], prediction[:, :, i])
                rmse = mean_squared_error(data_target_tensor[:, :, -1], prediction[:, :, i]) 
                mape = masked_mape_np(data_target_tensor[:, :, -1], prediction[:, :, i], 0)
            print('MAE: %.2f' % (mae))
            print('MSE: %.2f' % (rmse))
            print('MAPE: %.2f' % (mape))
            excel_list.extend([mae, rmse, mape])
            data_target_tensor = data_target_tensor[:,:,-1]
        ...
        if metric_method == 'mask':
            ...
            rmse = mean_squared_error(data_target_tensor.reshape(-1, 1), prediction.reshape(-1, 1)) 
            # rmse = torch.nn.MSELoss(data_target_tensor.reshape(-1, 1), prediction.reshape(-1, 1)) 
            mape = masked_mape_np(data_target_tensor.reshape(-1, 1), prediction.reshape(-1, 1), 0)
        print('all MAE: %.2f' % (mae))
        print('all MSE: %.2f' % (rmse))
